...
import { getRuralHistory, getRuralProjects } from '../../../store/actions';
import { getDropdownProjects } from '../../../store/actions';
import { useProjects } from '../project/useProjects';
import CreateNewRural from './create-new-rural';
...
const Rural = () => {
  ...
  useEffect(() => {
    if (record?.id) {
      dispatch(getRuralHistory({ pageIndex, pageSize, listType, recordId: record?.id }));
    }
  ...
  const { ruralProjects, ruralHistory } = useRural();
  const { data, count } = useMemo(
    () => ({
      data: ruralProjects.ruralProjectsObject?.rows || [],
      count: ruralProjects.ruralProjectsObject?.count || 0,
      isLoading: ruralProjects.loading || false
    }),
    [ruralProjects]
  );
  ...
  const { historyData, historyCounts } = useMemo(
    () => ({
      historyData: ruralHistory.ruralHistoryObject?.rows || [],
      historyCounts: ruralHistory.ruralHistoryObject?.count || 0,
      isLoading: ruralHistory.loading || false
    }),
    [ruralHistory]
  );
  ...
  const confirmDelete = async () => {
    const response = await request(`/delete-rural-hierarchies`, { method: 'DELETE', params: deleteId });
    if (response.success) {
      refreshPagination();
  ...
  const confirmRestore = async () => {
    ...
    const response = await request('/rural-hierarchies-update', { method: 'PUT', body: updatedValues, params: updatedValues.id });
    if (response.success) {
      refreshPagination();
    ...
    <>
      ...
      {selectedProject && (
        <TableForm
          ...
          exportConfig={{
            tableName: 'rural_hierarchies',
            fileName: 'rural-level',
