...
const CreateNewCustomer = (props) => {
  ...
  const onFormSubmit = async (values) => {
    let response;
    if (update) response = await request('/customer-update', { method: 'PUT', body: values, params: data.id });
    else response = await request('/customer-form', { method: 'POST', body: values });
    if (response.success) {
      refreshPagination();
  ...
  return (
    <>
      <FormProvider methods={methods} onSubmit={handleSubmit(onFormSubmit)}>
        <MainCard title={(view ? `View ` : update ? 'Update ' : 'Add ') + 'Customer'}>
          <Grid container spacing={4}>
            <Grid item md={3} xl={2}>
              {txtBox('name', 'Customer Name', 'text', true)}
            </Grid>
            <Grid item md={3} xl={2}>
              {txtBox('code', 'Customer Code', 'number', true)}
            </Grid>
            <Grid item md={3} xl={2}>
            ...
            <Grid item md={3} xl={2}>
              {txtBox('telephone', 'Telephone Number', 'number', true)}
            </Grid>
            <Grid item md={3} xl={2}>
