...
const MaterialInputs = ({ materials, onMaterailsInput, formData, selectedItems, view, update, showData, hideTaxDetails = false }) => {
    ...
    if (installersStoreData) {
      const newData = installersStoreData.filter(
        (val) =>
          ...
          ((formData.orgTypeId === '420e7b13-25fd-4d23-9959-af1c07c7e94b' &&
            (val?.name?.toLowerCase()?.includes('receiving') ||
              val?.name?.toLowerCase()?.includes('brought out') ||
              val?.name?.toLowerCase()?.includes('genus supply'))) ||
            (formData.orgTypeId === 'decb6c57-6d85-4f83-9cc2-50e0630003df' && val?.name?.toLowerCase()?.includes('receiving')))
      );
  ...
  useEffect(() => {
    ...
    if (projectStoreLocationData?.length > 0 && selectedStoreLocId !== null) {
      ...
      setValue('value', showData?.quantity ? parseFloat(quantityData.rate * showData.quantity).toFixed(2) : '0');
      ...
      <MainCard sx={{ mb: 2, pb: 4 }}>
        <FormProvider methods={methods}>
          ...
          <Grid container spacing={2} sx={{ mt: 2 }}>
            ...
            <Grid item xs={12} sm={4}>
              {selectBox('fromStoreLocationId', 'Receiving Store Location', filteredLocations, true, onStoreLocationSelected, false)}
            </Grid>
