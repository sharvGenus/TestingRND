...
const CreateNewUrban = (props) => {
  ...
  const onFormSubmit = async (values) => {
    values['levelType'] = 'urban';
    let response;
    if (update) {
      response = await request('/urban-hierarchies-update', { method: 'PUT', body: values, params: data.id });
    } else {
      response = await request('/urban-hierarchies-form', { method: 'POST', body: values });
    }
    if (response.success) {
  ...
  return (
    <>
      <FormProvider methods={methods} onSubmit={handleSubmit(onFormSubmit)}>
        <MainCard title={(view ? `View ` : update ? 'Update ' : 'Add ') + 'Urban Level'} sx={{ mb: 2 }}>
