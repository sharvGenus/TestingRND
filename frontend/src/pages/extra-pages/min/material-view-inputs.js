...
const MaterialInputs = ({
  ...
  useEffect(() => {
    setRate(selectedData && selectedData.rate ? selectedData?.rate : null);
  }, [selectedData]);
    ...
    resolver: yupResolver(
      Yup.object().shape({
        approvedQuantity: isInteger
          ? Validations.checkForInteger('Quantity', maxQuantity)
  ...
  useEffect(() => {
    ...
    if ((view || update) && selectedData) {
      ...
      if (selectedData.fromStoreLocationId && selectedData.fromStoreLocationId !== null) {
        dispatch(
          getSerialNumbers({
            project: selectedData?.projectId,
            store: selectedData?.fromStoreId,
            storeLocation: selectedData.fromStoreLocationId,
  ...
  }, [view, update, selectedData, dispatch, reset, setValue]);
  ...
  const onFormSubmit = (values) => {
    ...
    if (parseFloat(values.approvedQuantity) > parseFloat(values.qtyInStock)) {
  ...
  let { projectStoreLocationData, isLoading } = useMemo(
    () => ({
  ...
  useEffect(() => {
    if (isLoading) setReceivingData(true);
    else setReceivingData(false);
  }, [isLoading]);

  useEffect(() => {
    if (projectStoreLocationData && projectStoreLocationData.length > 0) {
      setLoading(false);
      setValue('qtyInStock', projectStoreLocationData[0].quantity);
      setValue('rate', parseFloat(projectStoreLocationData[0].rate).toFixed(2));
      setValue('tax', projectStoreLocationData[0].tax);
      setValue('value', parseFloat(projectStoreLocationData[0].rate * (quantity ? quantity : selectedData.approvedQuantity)).toFixed(2));
  ...
  }, [projectStoreLocationData, setValue, selectedData, quantity]);
          ...
          <Grid container spacing={2} sx={{ mt: 2 }}>
            ...
            <Grid item md={4}>
              {selectBox(
                'fromStoreLocationId',
                'From Store Location',
                // storeLocationData.filter((val) => !val.name.includes('Faulty') && !val.name.includes('Scrap')),
                storeLocationData &&
            ...
            <Grid item xs={1} mt={3.5} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
              {!view && (
                <Button size="small" onClick={handleSubmit(onFormSubmit)} variant="contained" color="primary" disabled={receivingData}>
                  {update ? 'Update' : 'Add'}
                </Button>
              )}
            </Grid>
