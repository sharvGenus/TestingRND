...
import { getGaaProjects } from 'store/actions';

const CreateNewGAA = (props) => {
  ...
  useEffect(() => {
    if (selectedProject) {
      dispatch(getGaaProjects({ projectId: selectedProject }));
    }
  ...
  const onFormSubmit = async (values) => {
    ...
    values['levelType'] = 'gaa';
    let response;
    if (update) {
      response = await request('/gaa-hierarchies-update', { method: 'PUT', body: values, params: data.id });
    } else {
      response = await request('/gaa-hierarchies-form', { method: 'POST', body: values });
    }
    if (response.success) {
  ...
  const onSubFormSubmit = async (values) => {
    ...
    values['isMapped'] = 1;
    let response;
    response = await request('/gaa-hierarchies-update', { method: 'PUT', body: values, params: values.mappingColumn });
    if (response.success) {
        ...
        <MainCard
          title={
            <Grid container xl={12} spacing={2} sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
              <Grid item xl={3} md={3}>
                <Typography variant="h4">{(view ? `View ` : update ? 'Update ' : 'Add ') + 'GAA Level'}</Typography>
              </Grid>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} scroll="paper" disableEscapeKeyDown>
        <FormProvider methods={subMethods} onSubmit={subMethods.handleSubmit(onSubFormSubmit)}>
          <Grid container spacing={3} sx={{ p: 2 }}>
            ...
            <Grid item md={6} xl={6}>
              {selectBox('mappingColumn', 'Mapping Column', gaaData)}
            </Grid>
