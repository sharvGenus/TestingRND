...
const CreateNewCancelPTPGRN = () => {
  ...
  const transactionTypeId = '73df529f-9008-4917-ab2b-088b9222fa68'; //Cancel PTP
  ...
  const makeRequest = () => {
    ...
    const stockLedgerDetails = {
      transactionTypeId: transactionTypeId,
      transactionCreatedAt: reqPTPData[0]?.createdAt,
      stoGrnIds: getIds(reqPTPData),
      stoGrnLedgerDetailId: reqPTPData?.[0]?.stock_ledger_detail?.id,
  ...
  const onFormSubmit = async (values) => {
    ...
    const resp = await request('/cancel-sto-grn-transaction-create', { method: 'POST', body: req });
  ...
  return (
    <>
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(onFormSubmit)}>
        <MainCard title={'Cancel PTPGRN(Project To Project GRN)'} sx={{ mb: 2 }}>
          <CancelTransactionRequestDropdownSection
            type="stockLedger"
            transactionType="PTPGRN"
...
export default CreateNewCancelPTPGRN;
