...
const MaterialInputs = ({ materials, onMaterailsInput, formData, selectedItems, view, update, showData, hideTaxDetails = false }) => {
    ...
    if ((view || update) && showData) {
      ...
      dispatch(
        getTxnByLocationMaterial({
          ...
          installerId: formData?.fromInstallerId,
      ...
      dispatch(
        getSerialNumbers({
          project: formData?.projectId,
          store: formData?.contractorStoreId,
          installer: formData?.fromInstallerId,
          storeLocation: showData?.fromStoreLocationId,
          material: showData?.materialId
        })
      );
  ...
  }, [installersStoreData, showData, formData]);
  ...
  const onStoreLocationSelected = (id, materialId) => {
    if (id) {
      setLoading(true);
      setSerials([]);
      setAllSerials([]);
  ...
  const onMaterialSelected = (e) => {
    if (e?.target?.value) {
      ...
      setValue('fromStoreLocationId', formData.toInstallerStoreLocationId);
      setSelectedStoreLocName(formData.toInstallerStoreLocationName);
      setSelectedStoreLocId(formData.toInstallerStoreLocationId);
      onStoreLocationSelected(formData.toInstallerStoreLocationId, e?.target?.value);
  ...
  useEffect(() => {
    ...
    if (projectStoreLocationData && projectStoreLocationData.length > 0 && selectedStoreLocId !== null) {
      ...
      const newQtyStk = toFixedQuantity(quantityData.quantity);
      const newRate = parseFloat(quantityData.rate).toFixed(2);
      const newTax = quantityData.tax;

      setValue('qtyStk', newQtyStk);

      if (alreadyAvailableItem) {
        setValue('quantity', alreadyAvailableItem.quantity);
        setSerials(alreadyAvailableItem.material_serial_numbers);
      } else {
        setValue('quantity', update && showData?.quantity ? showData.quantity : 0);
        setSerials(update && showData?.material_serial_numbers ? showData.material_serial_numbers : []);
      }
      ...
      if (showData?.quantity) {
        setValue('value', parseFloat(newRate * showData.quantity).toFixed(2));
      }
    } else {
      setValue('qtyStk', selectedStoreLocId !== null ? 0 : null);
      setValue('rate', '0');
      ...
      <MainCard sx={{ mb: 2, pb: 4 }}>
        <FormProvider methods={methods}>
          ...
          <Grid container spacing={2} sx={{ mt: 2 }}>
            ...
            <Grid item xs={12} sm={4}>
              {selectBox('fromStoreLocationId', 'From Store Location', filteredLocations, true, undefined, true)}
            </Grid>
