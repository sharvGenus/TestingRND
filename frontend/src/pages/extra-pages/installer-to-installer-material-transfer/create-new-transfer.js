...
const CreateNewMaterialTransferToInstaller = ({ disableAll, saveData }) => {
  ...
  const [fromInstaller, setFromInstaller] = useState(null);
  ...
  const methods = useForm({
    resolver: yupResolver(
      Yup.object().shape({
        ...
        fromInstallerId: Validations.fromInstaller,
        toInstallerId: Validations.toInstaller,
        toInstallerStoreLocationId: Validations.toStoreLocationId
      })
    ),
    defaultValues: {},
    mode: 'all'
  ...
  const resetContents = (stage) => {
    if (stage === 'projectId') {
      ...
      setValue('fromInstallerId', null);
      setValue('toInstallerId', null);
      setValue('toInstallerStoreLocationId', null);
      setAddress('');
    }
    if (stage === 'orgTypeId') {
      setValue('contractor', null);
    ...
    if (stage === 'fromInstallerId') {
      setValue('toInstallerId', null);
      setValue('toInstallerStoreLocationId', null);
    }
    if (stage === 'toInstallerId') {
      setValue('toInstallerStoreLocationId', null);
    }
  };
  ...
  const transactionTypeId = fetchTransactionType(transactionTypeData, 'ITI');
  ...
  const onFromInstallerSelected = (e) => {
    if (e?.target?.value) setFromInstaller(e?.target?.value);
  };
  ...
  return (
    <>
      <MainCard title={'Material Transfer Installer to Installer'} sx={{ mb: 2 }}>
        <FormProvider methods={methods}>
          <Grid container spacing={4}>
            <Grid item xs={12} sx={{ textAlign: 'right' }} mt={-9}>
              <APIImport tableName={'iti'} apipath={'/import-iti-transaction'} forTraxns />
            </Grid>
            <Grid item md={3} xl={2}>
            ...
            <Grid item md={3} xl={2}>
              {selectBox('fromInstallerId', 'From Installer', installerData, true, onFromInstallerSelected)}
            </Grid>
            <Grid item md={3} xl={2}>
              {selectBox(
                'toInstallerId',
                'To Installer',
                installerData.filter((vl) => vl.id !== fromInstaller && vl.isLocked === false),
                true,
                () => {
                  ...
                  setValue('toInstallerStoreLocationId', installersData && installersData.length ? installersData[0]?.id : null);
                  setValue('toInstallerStoreLocationName', installersData && installersData.length ? installersData[0]?.name : null);
                }
              )}
            </Grid>
            <Grid item md={3} xl={2}>
              {selectBox(
                'toInstallerStoreLocationId',
                'To Store Location',
