...
import { prepareResponseForReceipt, fetchTransactionType } from 'utils';
...
const STOReceipt = () => {
  ...
  const transactionTypeId = useMemo(() => {
    if (lovsData) {
      return fetchTransactionType(lovsData, 'STO');
    }
    return null;
  }, [lovsData]);
    ...
    <>
      ...
      {editAction && (
        <Dialog open={open} onClose={() => setOpen(false)} scroll="paper" maxWidth="xs" disableEscapeKeyDown>
          <EditModal rowData={rowData} setOpen={setOpen} refreshPagination={refreshPagination} title="STO" editPlaceOfSupply />
        </Dialog>
      )}
