...
const STCReceipt = () => {
      ...
      data:
        prepareResponseForReceipt(stockLedgerDetailList?.stockLedgerDetailListObject?.rows).map((item, index) => ({
          ...
          ...(item?.stock_ledgers[0]?.isCancelled === true
            ? { status: 'Cancelled' }
            : item?.stock_ledgers.find((it) => it.quantity > 0 && it.willReturn === true && it.isProcessed === false)
            ? { status: null }
            : item?.stock_ledgers.every((it) => it.willReturn === false)
            ? { status: null }
            : { status: 'Processed' }),
  ...
  const transactionTypeId = fetchTransactionType(lovsData, 'STC');
    ...
    <>
      ...
      {editAction && (
        <Dialog open={open} onClose={() => setOpen(false)} scroll="paper" maxWidth="xs" disableEscapeKeyDown>
          <EditModal rowData={rowData} setOpen={setOpen} refreshPagination={refreshPagination} title="STC" editPlaceOfSupply />
        </Dialog>
      )}
