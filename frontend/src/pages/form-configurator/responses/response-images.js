...
const ImageCard = ({ imageList, blockMode = false, base64Image = false, setValue, preview, type, mode }) => {
  ...
  const confirmDelete = () => {
    setValue(preview.columnName, type === 'blob' ? '' : imageList?.filter((x, i) => i !== selected));
    setSelected(0);
