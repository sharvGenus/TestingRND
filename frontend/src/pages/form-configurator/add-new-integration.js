...
import { getFormAttributeIntegrationCondition, getFormAttributes } from 'store/actions';
import { FormProvider, RHFSelectbox, RHFTextField } from 'hook-form';
import request from 'utils/request';
import Validations from 'constants/yupValidations';
import { formAttributesSlice, formAttributeIntegrationConditionSlice } from 'store/reducers/formMasterSlice';
...
const AddIntegrationForm = () => {
  ...
  const methods = useForm({
    resolver: yupResolver(
      Yup.object().shape({
        ...
        endpoint: Validations.other,
        type: Validations.other,
        method: Validations.other
      })
    ),
    defaultValues: {},
    mode: 'all'
  ...
  useEffect(() => {
    blockData?.id && dispatch(getFormAttributeIntegrationCondition(blockData?.id));
  }, [dispatch, blockData?.id]);

  const { attributeIntegrationCondition } = useDefaultFormAttributes();
  const attributeIntegrationConditionList = useMemo(
    () => attributeIntegrationCondition?.formAttributeIntegrationConditionObject?.rows || [],
    [attributeIntegrationCondition]
  );
  useEffect(() => {
    setConditionsArray((prevAttributes) => [...prevAttributes, ...attributeIntegrationConditionList]);
  }, [attributeIntegrationConditionList]);

  const onFormSubmit = async (values) => {
    values.conditionsArray = conditionsArray;
    values.formId = formId;
    const response = await request('/attribute-integration-block-create', { method: 'POST', body: values });
    if (response.success) {
      navigate(`/form-integration-rules/${formId}`);
    } else {
  ...
  const onFormUpdate = async (values) => {
    values.formId = formId;
    values.conditionsArray = conditionsArray;
    values.integrationBlockId = blockData?.id;
    const response = await request('/attribute-integration-block-update', { method: 'PUT', body: values, params: blockData?.id });
    if (response.success) {
      navigate(`/form-integration-rules/${formId}`);
    } else {
      toast(response.error.message, { variant: 'error' });
  ...
  useEffect(() => {
    ...
    setValue('endpoint', blockData?.endpoint);
    setValue('auth', blockData?.auth);
    setValue('type', blockData?.type);
    setValue('method', blockData?.method);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      dispatch(formAttributeIntegrationConditionSlice.actions.reset());
    };
  }, [dispatch, formId]);
    ...
    >
      <FormProvider methods={methods} onSubmit={handleSubmit(submitFunction)}>
        <Grid sx={{ position: 'absolute', top: 0, width: '100%', background: 'white', zIndex: 10, p: 2, pb: 0 }}>
          <Grid container>
            <Grid item md={12} sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Typography variant="h4">{!blockData?.id ? 'Add Integration Rule' : 'Edit Integration Rule'}</Typography>
        ...
        <Grid container spacing={4} sx={{ pl: 4, pr: 4, pt: 2, mt: 5, mb: 2 }}>
          ...
          <Grid item md={3} xl={3}>
            {txtBox(`endpoint`, 'API Endpoint', 'text', true, 'API Endpoint')}
          </Grid>
          <Grid item md={3} xl={3}>
            <RHFSelectbox name="method" placeholder="Select Method" label="Method" menus={methodsData} required={true} />
          </Grid>
          <Grid item md={3} xl={3}>
            <RHFSelectbox
              name="type"
...
const SubIntegrationForm = (props) => {
