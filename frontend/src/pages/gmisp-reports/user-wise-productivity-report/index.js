...
import { getDropdownProjects, getLovsForMasterName, getUserWiseProductivityReport, getWebformData } from 'store/actions';
...
function UserWiseProductivityReport() {
  const {
  ...
  const reportsLoading = useReports()?.userWiseProductivityReports?.loading;
  ...
  const columns = useMemo(
    () => [
      { Header: 'Project', accessor: 'projectName', exportAccessor: 'projectName' },
      { Header: 'Form Type', accessor: 'formTypeName', exportAccessor: 'formTypeName' },
      { Header: 'Form Name', accessor: 'formName', exportAccessor: 'formName' },
      { Header: 'Installer Name', accessor: 'Installer Name', exportAccessor: 'Installer Name' },
      { Header: 'Task Completed', accessor: 'Task Completed', exportAccessor: 'Task Completed' },
      { Header: 'L1 Approval Pending', accessor: 'L1 Approval Pending', exportAccessor: 'L1 Approval Pending' },
      { Header: 'L1 Approved', accessor: 'L1 Approved', exportAccessor: 'L1 Approved' },
      { Header: 'L1 Rejected', accessor: 'L1 Rejected', exportAccessor: 'L1 Rejected' },
      { Header: 'L1 On-Hold', accessor: 'L1 On-Hold', exportAccessor: 'L1 On-Hold' },
      { Header: 'L2 Approval Pending', accessor: 'L2 Approval Pending', exportAccessor: 'L2 Approval Pending' },
      { Header: 'L2 Approved', accessor: 'L2 Approved', exportAccessor: 'L2 Approved' },
      { Header: 'L2 Rejected', accessor: 'L2 Rejected', exportAccessor: 'L2 Rejected' },
      { Header: 'L2 On-Hold', accessor: 'L2 On-Hold', exportAccessor: 'L2 On-Hold' },
      { Header: 'Total Working Hours', accessor: 'Total Working Hours', exportAccessor: 'Total Working Hours' },
      { Header: 'Execution Started At', accessor: 'Execution Started At', exportAccessor: 'Execution Started At' },
      { Header: 'Execution Ended At', accessor: 'Execution Ended At', exportAccessor: 'Execution Ended At' }
    ],
    []
  );
  ...
  const loadData = useCallback(
    (values) => {
      ...
      dispatch(getUserWiseProductivityReport(payLoad));
    },
  ...
  useEffect(() => {
    ...
    loadData(values);
  }, [formId, formTypeId, getValues, loadData, projectId, showTable]);
  ...
  return (
    <MainCard title="User Wise Productivity Report">
      <FormProvider
        methods={methods}
      ...
      {showTable && (
        <TableForm
          ...
          title="User Wise Productivity Report"
          data={reportData}
          count={count}
          ...
          hideActions
          exportConfig={{
            tableName: 'user_wise_productivity_report',
            apiQuery: {
              projectId,
              formTypeId,
              formId,
...
export default UserWiseProductivityReport;
