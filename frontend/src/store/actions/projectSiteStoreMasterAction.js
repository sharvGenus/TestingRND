...
export const getProjectSiteStore = createAsyncThunk('getProjectSiteStore', async (object, { rejectWithValue }) => {
  ...
  const response = await request('/project-site-store-list', {
    method: 'GET',
...
export const getDropdownProjectSiteStore = createAsyncThunk('getDropdownProjectSiteStore', async (object, { rejectWithValue }) => {
  const response = await request('/project-site-store-dropdown', { method: 'GET' });
  if (response.success) {
    return response.data;
  }
  const error = response.error && response.error.message ? response.error.message : response.error;
