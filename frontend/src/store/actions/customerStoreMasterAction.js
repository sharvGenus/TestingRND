...
export const getCustomerStore = createAsyncThunk('getCustomerStore', async (object, { rejectWithValue }) => {
  ...
  const response = await request('/customer-stores-list', { method: 'GET', query: { sort: [sortBy, sortOrder], pageNumber, rowPerPage } });
  if (response.success) {
