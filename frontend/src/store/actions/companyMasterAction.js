...
export const getCompanies = createAsyncThunk('getCompanies', async (object, { rejectWithValue }) => {
  ...
  const response = await request('/company-list', { method: 'GET', query: { sort: [sortBy, sortOrder], pageNumber, rowPerPage } });
  if (response.success) {
...
export const getDropdownCompanies = createAsyncThunk('getDropdownCompanies', async (object, { rejectWithValue }) => {
  const response = await request('/company-dropdown', { method: 'GET' });
  if (response.success) {
    return response.data;
  }
  const error = response.error && response.error.message ? response.error.message : response.error;
