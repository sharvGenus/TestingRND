...
export const getSupplier = createAsyncThunk('getSupplier', async (object, { rejectWithValue }) => {
  ...
  const response = await request('/supplier-list', { method: 'GET', query: { sort: [sortBy, sortOrder], pageNumber, rowPerPage } });
  if (response.success) {
...
export const getDropdownSupplier = createAsyncThunk('getDropdownSupplier', async (object, { rejectWithValue }) => {
  const response = await request('/supplier-dropdown', { method: 'GET' });
  if (response.success) {
    return response.data;
  }
  const error = response.error && response.error.message ? response.error.message : response.error;
