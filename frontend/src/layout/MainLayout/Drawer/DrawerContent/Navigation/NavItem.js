...
const NavItem = ({ item, level }) => {
  ...
  const itemURL = `${item.url}${item.param ? `/${item.param}` : ''}${item.ctg ? `/${item.ctg}` : ''}`;
  let listItemProps = {
    component: forwardRef((props, ref) => <Link {...props} to={itemURL} target={itemTarget} ref={ref} />)
  };
  if (item?.external) {
