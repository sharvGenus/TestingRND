...
module.exports = {
    async up(queryInterface, Sequelize) {
        return queryInterface.sequelize.query(`
				DROP PROCEDURE IF EXISTS created_all_required_indexs;
				CREATE OR REPLACE PROCEDURE created_all_required_indexs(recreate boolean DEFAULT false)
				LANGUAGE plpgsql
				AS $$
				DECLARE
					rec RECORD;
	...
	AND IFS.TABLE_NAME = FT.TABLE_NAME
					...
					LOOP
						...
						-- query to generate migrations for history tables
						create_index_query := 'CREATE INDEX IF NOT EXISTS ' || rec.INDEX_NAME || '_history' ||
											' ON ' || rec.FACTORY_TABLE_NAME || '_history' || 
											' (' || rec.FACTORY_COLUMN_NAME || ');';
	...
	AND IFS.TABLE_NAME = MAPPER.TABLE_NAME
					...
					LOOP
						...
						-- Dynamically generate the CREATE INDEX query for history table
						create_index_query := 'CREATE INDEX IF NOT EXISTS ' || rec.INDEX_NAME || '_history' ||
											' ON ' || rec.MAPPING_TABLE || '_history' || ' (' || rec.MAPPING_COLUMN || ');';
	...
	AND IFS.TABLE_NAME = FORMS.TABLE_NAME
					...
					LOOP
						...
						create_index_query := 'CREATE INDEX IF NOT EXISTS ' || rec.INDEX_NAME || '_history' ||
											' ON ' || rec.FORM_NAME || '_history' || ' USING GIN (' || rec.GAA_COLUMNS || ');';
					...
					LOOP
						...
						-- history table
						create_index_query := 'CREATE INDEX IF NOT EXISTS ' || rec.INDEX_NAME || '_history' ||
											' ON ' || rec.FORM_NAME || '_history' || ' USING GIN (' || rec.APPROVAL_COLUMNS || ');';
					...
					LOOP
						...
						-- history
						create_index_query := 'CREATE INDEX IF NOT EXISTS ' || rec.INDEX_NAME || '_history' ||
											' ON ' || rec.FORM_NAME || '_history' || ' (' || rec.TIMESTMP_COLUMNS || ');';
				...
				CALL created_all_required_indexs(true);
			`);
    /**
