    ...
    async up(queryInterface, Sequelize) {
        ...
        BEGIN
            IF installerid IS NOT NULL AND projectid IS NOT NULL THEN
                RETURN QUERY SELECT sn.id, sn.material_id, sn.serial_number, m.material_type_id
				...
				IF formid IS NOT NULL AND fromattributeid IS NOT NULL AND responseid IS NOT NULL THEN
					...
					IF tablename IS NOT NULL THEN
						...
						IF array_length(materialidarr, 1) > 0 THEN
							-- Consumed
							RETURN QUERY
							SELECT sn.id, sn.material_id, sn.serial_number, m.material_type_id
							FROM "material_serial_numbers" sn
							INNER JOIN "materials" AS m ON sn.material_id = m.id
        ...
        BEGIN
            IF installerid IS NOT NULL AND projectid IS NOT NULL THEN
                RETURN QUERY 
				SELECT DISTINCT ON (sl.material_id) m.id, m.name, m.code, m.material_type_id, SUM(sl.quantity) AS quantity
